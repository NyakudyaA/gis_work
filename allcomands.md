...
* 2 doing gdal_translate using the original datasets
...
* 8 May 2013 postgis dump for a table
...
*  SQLite -dsco SPATIALITE=NO db.sqlite input.shp", 

* running spatialite use the following
...
*  Using raw imagery for level 0 in the case of cct2011_lo19
  I had to create a mosaic first to see if the ecw were working as the updating script did not work as it contained headers for the projection definition. so what i did was to create a gdaltindex and then upload the shapefile into the directory, i firstly tried to generate a qix file in qgis 
and uploading it into the directory as well as a prj file but it ended up not working and then i put the shapefile(.shx and .shp) and let it generate it index as well as prj file. I had to edit the prj file and add the correct definition and then resave the store and it worked, and i transfered it to the base 0 directory.
